...
async function displayMessage2(sender, message, timestamp) {
  ...
  messageElement.innerHTML = `<span class="sender">${sender}:</span> ${message} <span class="timestamp"> - ${getCurrentTime(
    timestamp
  )}</span>`;
...
async function displayMessage(sender, message, timestamp) {
  ...
  if (data) {
    ...
    messagesArray.forEach(({ name, message, timestamp }) => {
      var messageElement = document.createElement("div");
      messageElement.classList.add("message");
      messageElement.innerHTML = `<span class="sender">${name}:</span> ${message} <span class="timestamp"> - ${getCurrentTime(
        timestamp
      )}</span>`;
      messagesContainer.appendChild(messageElement);
    });
